...
pub fn solution() -> Int {
  ...
  set.fold(zeros, 0, fn(acc, zero) {
    acc + set.size(trailhead(set.new(), zero, 0, map))
  })
}
...
fn trailhead(
  ...
  case dict.get(map, position) {
    Ok(n) if n == expected -> {
      use <- bool.guard(n == 9, set.insert(found, position))

      found
      |> trailhead(move(position, Up), expected + 1, map)
      |> trailhead(move(position, Right), expected + 1, map)
      |> trailhead(move(position, Down), expected + 1, map)
      |> trailhead(move(position, Left), expected + 1, map)
    }
    _ -> found
  }
}
